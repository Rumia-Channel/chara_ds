...
- Actor Guard: 任意の第三者監視役。`deepseek-v4-flash` で年齢・身体能力・口調の不整合を判定し、問題があれば「何がどうおかしいか」をActorへ返して同じターンを書き直させる。
...
- `--actor-guard` を付けると、各Actor出力後に第三者視点のFlash監視を挟みます。既定は `--actor-guard-model deepseek-v4-flash --actor-guard-thinking off` です。
...
- `--actor-guard` adds a third-person Flash judge after each Actor turn. By default it uses `--actor-guard-model deepseek-v4-flash --actor-guard-thinking off`.
