...
- Age/Gender Norms: `age_gender_norms/` の属性別 JSON から必要な年齢・性別・反応ガイドだけを Persona Controller と Actor Guard に渡し、普通寄りの人物が不自然に粗野化しないようにする。
...
`age_gender_norms/` が prompt directory にある場合は、Persona Controller と Actor Guard の共通参照データとして読み込まれます。これは lax/通常で分ける創作強度ではなく、年齢・性別・属性ごとの一般的な反応基準です。

- `age_gender_norms/index.json`: 利用可能な属性一覧。例: 女子中学生、女子高校生、男子中学生、成人女性。
- `age_gender_norms/*.json`: 属性ごとの一人称・二人称、悲鳴、痛み、制止、怒り語彙、避ける粗野表現、Actor Guard の判定規則。
...
これにより、普通寄りの女子中学生が攻撃された直後に、悲鳴や痛みの声なしで成人男性的な罵倒へ飛ぶような崩れを抑えつつ、全属性データを毎回 payload に積んでトークンを浪費しないようにしています。
